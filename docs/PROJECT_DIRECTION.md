...
- 使用 Spring Boot 3、MySQL 5.7 和 RocketMQ 的 Java 后端应用；
...
`0.1.0` 只支持 MySQL 5.7，因此不公开通用存储插件接口。存储实现作为模块内部实现存在，避免只有一个生产适配却提前设计抽象。
...
- `version` 用于 MySQL 5.7 条件更新抢占；
- `last_error` 只保存截断后的异常摘要，不保存完整敏感数据；
- 已发布事件按照保留时间批量归档或删除，不无限增长。

## 10. MySQL 5.7 抢占策略

MySQL 5.7 不依赖 `SKIP LOCKED`。发布器先按索引分页获取候选 ID，再逐条或小批量执行条件更新：
...
使用 Testcontainers 启动 MySQL 5.7：
...
> 设计并实现基于 Transactional Outbox 的 RocketMQ 可靠消息 Spring Boot Starter，使业务数据与待发布事件在同一本地事务中提交；基于 MySQL 5.7条件更新和任务租约实现多实例事件抢占、失败重试及宕机恢复，并在优惠券任务创建链路完成落地验证。
...
- 已发布事件采用删除还是归档；
- 是否在 `0.2.0` 增加人工重放接口；
- 是否在 `0.2.0` 增加 MySQL 8 `SKIP LOCKED` 策略。
