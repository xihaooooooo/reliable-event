...
> 项目目前完成到 M4.5：数据库侧实现事务内登记、版本号和租约抢占、指数退避、死信及进程退出恢复；发送侧接入 RocketMQ 5.x gRPC 客户端，并用真实 Broker 验证消息映射、暂停恢复和结果未知后的重复投递。Starter 能自动持续扫描、有界并发发送及限时优雅停机，并通过 Micrometer 记录发送、积压、死信和租约恢复指标。下一步是原创示例、业务私有接入及基准测试。项目采用至少一次语义，下游仍需按稳定事件 ID 或业务 Key 去重。
