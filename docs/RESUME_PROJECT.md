...
适用状态：已经完成 JDBC 可靠性协议、RocketMQ 发送适配、Starter 自动配置和真实 Broker 故障测试，但尚未完成常驻调度。
...
- 增加 Spring Boot 条件装配、配置校验、目标映射绑定和 Producer Bean 生命周期；使用 MySQL 8.0.36、RocketMQ 5.5.0 和独立 JVM 故障注入完成 107 个自动化测试，覆盖双 Worker 竞争、进程退出、Broker 暂停恢复，以及 Starter 真实服务装配。

当前 Starter 已可装配并显式执行一轮发布，但尚未完成后台调度、有界并发、优雅停机、指标和业务项目接入，因此不要描述为引入依赖后即可自动持续发布。
...
> 项目目前完成到 M4.2：数据库侧已经实现事务内登记、版本号和租约抢占、指数退避、死信及进程退出恢复；发送侧接入 RocketMQ 5.x gRPC 客户端，并用真实 RocketMQ 5.5.0 验证消息映射、Broker 暂停恢复和结果未知后的重复投递。Starter 已能自动装配并显式执行一轮发布，下一步是常驻调度与有界并发。项目采用至少一次语义，下游仍需按稳定事件 ID 或业务 Key 去重。
