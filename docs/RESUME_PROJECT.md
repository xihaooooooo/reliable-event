...
> 项目目前已经完成完整 M3：业务事务可以登记 Outbox 事件，后台发布器通过 MySQL 8.0 版本号条件更新和数据库时间租约抢占事件；普通失败会指数退避重试，达到最大次数或遇到不可重试错误会进入死信。过期租约支持并发安全恢复，并通过独立 JVM 强制退出测试验证抢占后进程退出可以由新 Worker 接管，也稳定复现了发送成功但状态未更新时的重复投递窗口。现在还没有接入真实 RocketMQ 和 Starter，下一步是实现 RocketMQ 发送适配与目标映射。
