...
适用状态：已经完成 JDBC 可靠性协议、RocketMQ 发送适配、Starter 自动配置、常驻调度、有界并发和有界优雅停机，以及真实 Broker 故障测试；尚未完成生产指标。
...
- 增加 Spring Boot 条件装配、固定延迟调度、有界并发和限时优雅停机；候选在线程开始执行后才抢占租约，关闭时先撤销队列再等待在途发送。使用 MySQL 8.0.36、RocketMQ 5.5.0 和独立 JVM 故障注入完成 120 个自动化测试，覆盖双 Worker 竞争、进程退出、Broker 暂停恢复、自动发送、排队容量和超时恢复。

当前 Starter 已能自动持续发布并有界等待在途发送；尚未完成指标和业务项目接入，不要描述为完整生产级方案。
