...
适用状态：已经完成 JDBC 可靠性协议、RocketMQ 发送适配和真实 Broker 故障测试，但尚未完成 Starter 自动配置与常驻调度。

### ReliableEvent——基于 Transactional Outbox 的 RocketMQ 可靠消息组件（个人开源项目）
...
- 使用 MySQL 8.0.36、RocketMQ 5.5.0 和独立 JVM 故障注入完成 91 个自动化测试，覆盖双 Worker 竞争、进程退出、Broker 暂停恢复，以及成功回执丢失后产生不同 Message ID 的预期重复消息。

当前尚未完成 Spring Boot 自动配置、后台调度、有界并发、优雅停机、指标和业务项目接入，因此不要写成已经完成的 Starter。
...
> 项目目前完成到 M4.1：数据库侧已经实现事务内登记、版本号和租约抢占、指数退避、死信及进程退出恢复；发送侧已经接入 RocketMQ 5.x gRPC 客户端，并用真实 RocketMQ 5.5.0 验证消息映射、Broker 暂停恢复和结果未知后的重复投递。项目明确采用至少一次语义，下游仍需按稳定事件 ID 或业务 Key 去重。现在还没有完成 Starter 自动配置和常驻调度，下一步是 M4.2 的 Spring Boot 装配。
