# M5.2：私有优惠券项目首条业务链路接入

> 状态：代码已接入并通过聚焦集成测试；端到端业务幂等和真实环境故障演练仍待验收。实施证据见[进度记录](../progress/M5_2_IMPLEMENTATION_PROGRESS.md)。
>
> 基线：M5.1 已完成；ReliableEvent 使用 Java 17、Spring Boot 3.5.16、MySQL 8.0、RocketMQ 5.x 普通消息
>
> 目标：在私有优惠券项目的“创建发券任务”链路中，让 `t_coupon_task` 与 Outbox 事件在同一个本地事务内提交，由 Starter 在任务到期后发布原有业务消息，并用实际业务结果证明故障恢复与重复消费安全。

M5.1 的原创示例已经证明 Starter 可以被一个独立应用接入。M5.2 要验证真实业务链路，而不是再写一套演示应用。私有优惠券项目位于本机 `D:\trae\onecoupon\onecoupon`，保持在原仓库；本仓库只保存接入协议和脱敏验收结果，不复制其业务源码、数据库数据或凭据。本阶段只覆盖[项目方向文档](../PROJECT_DIRECTION.md#19-优惠券项目落地计划)中的第一阶段；预约提醒和用户券过期事件不进入本阶段。
